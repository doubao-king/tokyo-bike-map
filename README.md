...
- 駅・施設・住所から目的地を検索し、2km以内の駐輪場を近い順に表示
- 主要14駅の検索向け駐輪場ページと、選んだ目的地を保つ共有リンク
- 現在の地図位置を添付する、アカウント不要の非公開修正フォーム
- typed GeoJSONデータを差し替えるだけで実データ化できる構成
- 区間ごとの出典・確認日・整備状況の表示
- モバイル対応
- このマップについて、データ方針、更新履歴、プライバシー、利用規約の公開ページ
...
道路と駐輪場の地図データは静的です。OSMと行政データの取得・正規化は開発時に実行し、Vite buildでは生成済みGeoJSONをそのまま配信します。目的地検索だけはブラウザからStadia MapsのジオコーディングAPIを利用し、選択地点と静的駐輪場データの直線距離をブラウザ内で計算します。検索語や検索結果は当サイトのデータベースへ保存しません。閲覧カウンターと非公開の修正報告にはCloudflare WorkerとD1を使用します。

## ロードマップ

当面は、モバイルを含む使いやすさ、データ更新、透明性、共有しやすさ、簡単な修正受付を改善し、利用者コミュニティの土台を作ります。

将来は、住民が地図上の課題を事実に基づいて整理し、適切な行政・議会の公式窓口へ届けられる機能を検討します。公開投票や議員への自動送信を先に導入せず、プライバシー、モデレーション、中立性を重視します。

詳しい段階と原則は [ROADMAP.md](ROADMAP.md) に記録しています。
