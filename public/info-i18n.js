...
const pages = {
  en: {
    ...
    updates: {
      ...
      html: `<section><h2>27 August 2026</h2><ul><li>Added Japanese, English and Chinese interfaces.</li><li>Added current-location display.</li><li>Added the GSI slopes and terrain layer.</li><li>Added 672 bicycle-parking facilities from official datasets published by 22 municipalities.</li></ul></section><section><h2>26 August 2026</h2><ul><li>Added public hosting, security settings and a commercial map-tile provider.</li><li>Added public pages covering methodology, privacy and terms.</li><li>Organized Tokyo-wide area shortcuts and official references.</li></ul></section><section><h2>25 August 2026</h2><ul><li>Imported Tokyo-wide cycling-related OpenStreetMap data.</li><li>Omitted short bicycle-crossing strokes.</li><li>Resolved overlaps between mixed-traffic and more comfortable classes.</li></ul></section><p class="updated">Data is refreshed according to source availability and review capacity.</p>`
    },
    contact: {
      description: 'How to suggest corrections or contact Tokyo Bicycle Map.',
      lede: 'If the map differs from conditions on site, or a source needs correction, please open a public issue.',
      html: `<section><h2>Report a correction</h2><p><a href="https://github.com/doubao-king/tokyo-bike-map/issues/new" rel="noreferrer">Create a new GitHub issue</a></p><p>The following details help us review it:</p><ul><li>A shared map URL, road name or approximate location</li><li>What the map shows and what is present on site</li><li>The date observed</li><li>A publicly shareable photo or official source URL</li></ul></section><section><h2>Do not publish personal information</h2><div class="notice"><p>Do not post names, addresses, phone numbers, identifiable faces or vehicle registration numbers in an issue.</p></div></section><section><h2>Review</h2><p>Suggestions are checked against public sources and map data before inclusion. We may not be able to incorporate every suggestion, and verification can take time.</p></section>`
    },
    privacy: {
      description: 'How Tokyo Bicycle Map handles site settings, location, view counts and external services.',
      lede: 'Tokyo Bicycle Map does not require an account or ask for your name or address.',
      html: `<section><h2>Current data use</h2><dl class="definition-list"><dt>Site settings</dt><dd>Map position, road categories and layers are included in shareable URLs. Only the selected language is stored in local storage.</dd><dt>Current location</dt><dd>Your browser's location feature is used only after you press Show my location. The position is used in the browser to move the map and is not sent to or stored by this site's server.</dd><dt>View counter</dt><dd>Cloudflare D1 stores only a site-wide total. Session storage prevents repeated reloads in the same tab from being counted. No person-specific history or identifier is stored in the database.</dd><dt>Hosting</dt><dd>Cloudflare may process IP addresses and ordinary request information for delivery, security and incident response.</dd><dt>Map tiles</dt><dd>Stadia Maps receives technical request information when loading the base map. GSI receives it when the terrain layer is enabled.</dd><dt>Advertising</dt><dd>Google AdSense loads only after site review, consent settings and ad IDs are complete. When enabled, Google may process cookies, IP addresses and browser information for ad delivery and measurement.</dd><dt>Analytics</dt><dd>No analytics service that tracks individual behavior is enabled.</dd></dl></section><section><h2>External links</h2><p>External sites such as OpenStreetMap, government agencies and GitHub apply their own privacy policies.</p></section><section><h2>Future changes</h2><p>This page will be updated before changing external services or stored information.</p></section><section><h2>Contact</h2><p>Questions about this policy can be sent through <a href="/contact/index.html?lang=en">Corrections and contact</a>.</p><p class="updated">Effective: 26 August 2026 / Updated: 27 August 2026</p></section>`
    },
    terms: {
  ...
  zh: {
    ...
    methodology: {
      ...
      html: `<section><h2>当前地图数据</h2><p>彩色路段是构建时根据 OpenStreetMap 道路形状和标签提取、分类的静态 GeoJSON。使用地图时，浏览器不会查询 OpenStreetMap 或政府网站。</p><p>东京都过去的推荐路线和优先建设路段仅作为调查资料保存，不作为当前的舒适道路显示。</p></section><section><h2>附加图层</h2><dl class="definition-list"><dt>坡度与地形</dt><dd>国土地理院的坡度图显示地形陡峭程度，不代表单条道路的方向或精确坡度。</dd><dt>自行车停车场（官方数据）</dt><dd>将各地方政府通过东京都开放数据目录发布的位置资料静态导入。设施运营方可能不同。由于公开格式和坐标情况不同，目前尚未覆盖所有地方政府和设施。</dd></dl></section><section><h2>道路分类</h2><dl class="definition-list"><dt>与机动车物理隔离</dt><dd>通过路缘、护栏或绿化等与机动车交通分隔的骑行空间。</dd><dt>宽阔人行道、绿道和河岸</dt><dd>包括宽阔人行道内的骑行空间、绿道和河岸，部分路段与行人共用。</dd><dt>自行车专用车道</dt><dd>机动车道上的专用通行带，包括没有物理隔离的路段。</dd><dt>混合车流</dt><dd>设置自行车引导线或标记的机动车道，初始状态不显示。</dd></dl></section><section><h2>限制</h2><ul><li>根据 OSM 标签推测的分类并非官方安全评价。</li><li>在找到合适的表现方式前，短小的自行车横穿带暂不显示。</li><li>混合车流与更舒适分类重合时，会移除重合部分。</li><li>道路宽度、交通量、道路坡度、照明和路面状况不影响舒适度分类；地形图层也不改变分类。</li></ul></section><section><h2>来源与许可</h2><p>行政资料列在地图的“数据来源”中。OpenStreetMap 数据依据 Open Data Commons Open Database License 使用。</p></section>`
    },
    updates: {
      description: '东京自行车地图的数据与功能更新记录。',
      lede: '本页记录数据日期以及影响分类或显示方式的变更。',
      html: `<section><h2>2026年8月27日</h2><ul><li>增加日语、英语和中文界面。</li><li>增加当前位置显示。</li><li>增加国土地理院的坡度与地形图层。</li><li>增加22个地方政府官方数据中的672处自行车停车设施。</li></ul></section><section><h2>2026年8月26日</h2><ul><li>增加公开托管、安全设置和商用地图图块服务。</li><li>增加数据方法、隐私政策和使用条款等页面。</li><li>整理覆盖东京全域的地区按钮与官方参考资料。</li></ul></section><section><h2>2026年8月25日</h2><ul><li>导入东京全域的 OpenStreetMap 自行车相关数据。</li><li>暂不显示短小的自行车横穿带。</li><li>整理混合车流与更舒适分类的重合部分。</li></ul></section><p class="updated">数据将根据来源的公开情况和核查能力进行更新。</p>`
    },
    contact: {
      description: '如何向东京自行车地图提出修正或联系。',
      lede: '如地图与现场状况不符，或数据来源需要修正，请提交公开 Issue。',
      html: `<section><h2>报告修正</h2><p><a href="https://github.com/doubao-king/tokyo-bike-map/issues/new" rel="noreferrer">在 GitHub 新建 Issue</a></p><p>提供以下信息有助于核查：</p><ul><li>地图分享链接、道路名称或大致位置</li><li>地图显示内容与现场实际情况</li><li>确认日期</li><li>可公开的照片或官方资料链接</li></ul></section><section><h2>请勿公开个人信息</h2><div class="notice"><p>请勿在 Issue 中发布姓名、地址、电话号码、可识别人脸或车辆号码的照片。</p></div></section><section><h2>核查与反映</h2><p>建议将在与公开资料和地图数据核对后反映。我们可能无法采纳所有建议，核查也可能需要时间。</p></section>`
    },
    privacy: {
      description: '东京自行车地图如何处理网站设置、位置、浏览次数和外部服务。',
      lede: '东京自行车地图不要求注册账号，也不会要求输入姓名或地址。',
      html: `<section><h2>当前的数据使用</h2><dl class="definition-list"><dt>网站设置</dt><dd>地图位置、道路分类和图层会包含在可分享的链接中。浏览器本地存储只保存所选语言。</dd><dt>当前位置</dt><dd>仅在按下“显示当前位置”后使用浏览器的位置功能。位置只在浏览器内用于移动地图，不会发送或保存到本网站服务器。</dd><dt>浏览计数</dt><dd>Cloudflare D1 只保存全站累计次数。会话存储用于避免同一标签页重复刷新计数。数据库不保存个人浏览记录或识别码。</dd><dt>托管</dt><dd>Cloudflare 可能为内容分发、安全防护和故障处理而处理 IP 地址及一般请求信息。</dd><dt>地图图块</dt><dd>加载底图时 Stadia Maps 会收到技术请求信息；启用地形图层时国土地理院会收到相应信息。</dd><dt>广告</dt><dd>仅在网站审核、同意设置和广告 ID 完成后加载 Google AdSense。启用时，Google 可能为广告投放和效果测量处理 Cookie、IP 地址和浏览器信息。</dd><dt>访问分析</dt><dd>目前未启用追踪个人行为的分析服务。</dd></dl></section><section><h2>外部链接</h2><p>OpenStreetMap、政府机关和 GitHub 等外部网站适用其各自的隐私政策。</p></section><section><h2>今后变更</h2><p>如外部服务或保存的信息发生变化，本页会在实际启用前更新。</p></section><section><h2>联系</h2><p>关于本政策的问题可通过<a href="/contact/index.html?lang=zh">信息修正与联系</a>提出。</p><p class="updated">制定：2026年8月26日 / 更新：2026年8月27日</p></section>`
    },
    terms: {
...
if (header) {
  ...
  control.innerHTML = `<span class="visually-hidden">${labels[language].language}</span><select aria-label="${labels[language].language}"><option value="ja">日本語</option><option value="en">English</option><option value="zh">中文</option></select>`;
  const select = control.querySelector('select');
  select.value = language;
  select.addEventListener('change', () => {
    try {
      window.localStorage.setItem('tokyo-bike-map-language', select.value);
    } catch {
      // The query parameter still carries the choice.
    }
    const url = new URL(window.location.href);
    if (select.value === 'ja') url.searchParams.delete('lang');
