...
const labels = {
  ...
  zh: {
    about: '关于这张地图', contact: '信息修正与联系', language: '显示语言',
...
const pages = {
  en: {
    about: {
      description: 'A map where Tokyo cyclists can share what they find and, over time, help make the case for safer cycling infrastructure.',
      lede: 'When you cycle in Tokyo, some streets feel easy and others feel frightening. We want this to become a map where people can share both.',
      html: `<section><h2>First, make a useful map</h2><p>Today you can compare roads separated from traffic, greenways and bicycle lanes, find parking near a destination, and report something that differs on site without creating an account.</p><p>Some areas still have little information, and some details will be wrong. Rather than pretend the map is complete, we want to improve it gradually with the people who actually cycle in Tokyo.</p></section><section><h2>Where it might lead</h2><p>As more people use the map and share local information, we may be able to show where bicycle lanes end abruptly, where riding near cars feels frightening, and where bicycle parking is missing.</p><p>Someday, those patterns could help cyclists talk to road managers, local governments and elected representatives, and make a stronger case for safer cycling infrastructure in Tokyo.</p><p>We do not yet know what form would genuinely be useful. Instead of deciding on a large system now, we will start by growing a map people use and work out the next step together with Tokyo's cycling community.</p></section><section><h2>Follow conditions on site</h2><div class="notice"><p>Roadworks, time restrictions, sign changes, congestion and surface conditions may not be reflected. Always follow signs, traffic rules and conditions on site.</p></div></section>`
    },
    methodology: {
  ...
  zh: {
    about: {
      description: '一张让东京骑行者分享现场信息，并逐步为更安全的自行车基础设施提供支持的地图。',
      lede: '在东京骑车时，有些道路让人轻松，也有些地方让人害怕。我们希望这张地图可以让大家把这些感受和信息分享出来。',
      html: `<section><h2>先把地图做得真正有用</h2><p>现在可以比较与机动车隔离的道路、绿道和自行车专用车道，查找目的地附近的停车场，也可以在无需账号的情况下报告与现场不符的信息。</p><p>有些地区的信息仍然很少，也一定会有错误。我们不想假装这是一张完美的地图，而是希望和真正在东京骑车的人一起，慢慢把它做得更有用。</p></section><section><h2>以后也许能做到的事</h2><p>当使用地图的人和现场信息逐渐增加，我们也许可以更清楚地看到自行车道突然中断、机动车距离过近、停车场不足等问题。</p><p>希望有一天，这些信息可以成为骑行者与道路管理部门、地方政府和议员沟通的材料，为东京建设更安全的自行车基础设施提供一点推动力。</p><p>我们还不知道怎样的形式才真正有用。与其现在就决定一个庞大的制度，不如先把地图做好，并与东京的骑行者一起思考下一步。</p></section><section><h2>请以现场状况为准</h2><div class="notice"><p>道路施工、时段限制、标志变化、拥堵和路面状况可能尚未反映。骑行时请遵守现场标志、交通规则并留意周围环境。</p></div></section>`
    },
    methodology: {
    ...
    privacy: {
      ...
      html: `<section><h2>当前的数据使用</h2><dl class="definition-list"><dt>网站设置</dt><dd>地图位置、道路分类和图层会包含在可分享的链接中。浏览器本地存储只保存所选语言。</dd><dt>当前位置</dt><dd>仅在按下“显示当前位置”后使用浏览器的位置功能，用于在浏览器内移动地图。如随后发送修正报告，会保存报告所指的地图坐标。</dd><dt>目的地搜索</dt><dd>搜索文字会发送至 Stadia Maps 以获取候选结果，Stadia Maps 可能处理 IP 地址、浏览器信息等技术信息。所选地点与停车场之间的直线距离在浏览器内计算。本网站的数据库不会保存搜索文字或搜索结果。</dd><dt>修正报告</dt><dd>Cloudflare D1 会以非公开方式保存报告类型、正文、选填的确认日期、显示语言、地图 URL 和坐标。选择道路或自行车停车场时，还会保存对象类型、数据 ID 和显示名称。报告数据表不保存姓名、电子邮箱、账号、IP 地址或浏览器信息。报告会在核查和修正所需期间保留，不再需要时可以删除。请勿填写个人信息。</dd><dt>浏览计数</dt><dd>Cloudflare D1 只保存全站累计次数。会话存储用于避免同一标签页重复刷新计数。数据库不保存个人浏览记录或识别码。</dd><dt>托管</dt><dd>Cloudflare 可能为内容分发、安全防护和故障处理而处理 IP 地址及一般请求信息。</dd><dt>地图图块</dt><dd>加载底图时 Stadia Maps 会收到技术请求信息。</dd><dt>广告</dt><dd>仅在网站审核、同意设置和广告 ID 完成后加载 Google AdSense。启用时，Google 可能为广告投放和效果测量处理 Cookie、IP 地址和浏览器信息。</dd><dt>访问分析</dt><dd>目前未启用追踪个人行为的分析服务。</dd></dl></section><section><h2>外部链接</h2><p>OpenStreetMap、政府机关等外部网站适用其各自的隐私政策。</p></section><section><h2>今后变更</h2><p>如外部服务或保存的信息发生变化，本页会在实际启用前更新。</p></section><section><h2>联系</h2><p>关于本政策的问题可通过<a href="/contact/?lang=zh">信息修正与联系</a>提出。</p><p class="updated">制定：2026年8月26日 / 更新：2026年8月31日</p></section>`
    },
    terms: {
...
function renderNavigation() {
  ...
  ['about', 'methodology', 'updates', 'contact', 'privacy', 'terms']
    ...
    .forEach((id) => {
      ...
      link.href = `/${id}/${languageQuery(language)}`;
